package controller.concat_WAVs_To_WAVsOrMP3s;
 ...
 */
public class ConcatWithNoUI implements ControllerListener, DataSinkListener {
	...
	/**
	 * can not merge mp3 with 256 birate can not merge to mp3
	 * 
	 * @param args
	 * @return
	 */
	public static boolean merge(String args[]) {
		...
		ConcatWithNoUI concat = new ConcatWithNoUI();
	...
	public static void main(String[] args) {
		String arg[] = { "-o", "file:/c:/foo.wav", "file:/c:/1.mp3",
				"file:/c:/1.wav" };
		args = arg;
		Vector<String> inputURL = new Vector<String>();
		String outputURL = null;
	...
	public boolean matchTracks(ProcInfo pInfo[], ContentDescriptor cd) {
		...
		if (total[AUDIO] < 1 && total[VIDEO] < 1) {
			System.err
					.println("There is no audio or video tracks to concatenate.");
			return false;
		}
					...
					if (oldFmt instanceof VideoFormat) {
						...
						if (oldSize != null && !oldSize.equals(newSize)) {
							...
							System.err.println("  to: " + newSize.width + " x "
									+ newSize.height);
							newFmt = (new VideoFormat(null, newSize,
			...
			for (int type = AUDIO; type < MEDIA_TYPES; type++) {
				for (int i = 0; i < pInfo.numTracksByType[type]; i++) {
					if (!masterFound && isRawAudio(pInfo.tracksByType[type][i])) {
						...
						streams[j]
								.setStream(pInfo.tracksByType[type][i], false);
					j++;
				}
			}
		}
