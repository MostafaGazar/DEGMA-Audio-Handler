...
import view.sideBar.plaf.ButtonBarButtonUI;
import view.sideBar.plaf.ButtonBarUI;

/**
 * JButtonBar helps organizing buttons together (as seen in Mozilla Firefox or
 * IntelliJ).<br>
 * 
 * @javabean.class name="JButtonBar" shortDescription=
 *                 "JButtonBar helps organizing buttons together (as seen in Mozilla Firefox or IntelliJ)."
 *                 stopClass="java.awt.Component"
 * 
 * @javabean.icons mono16="JButtonBar16-mono.gif" color16="JButtonBar16.gif"
 *                 mono32="JButtonBar32-mono.gif" color32="JButtonBar32.gif"
 */
public class JButtonBar extends JComponent implements SwingConstants {
	 ...
	 * 
	 * @param ui
	 *            the <code>ButtonBarUI</code> L&F object
	 ...
	 * 
	 * @param orientation
	 *            the new orientation -- either <code>HORIZONTAL</code>
	 *            or</code> VERTICAL</code>
	 * @exception IllegalArgumentException
	 *                if orientation is neither <code>
	 *              HORIZONTAL</code> nor <code>VERTICAL</code>
	 * @see #getOrientation @beaninfo description: The current orientation of
	 *      the bar bound: true preferred: true
	 */
	public void setOrientation(int orientation) {
		...
		if (this.orientation != orientation) {
			...
			this.orientation = orientation;
			firePropertyChange(ORIENTATION_CHANGED_KEY, oldOrientation,
					this.orientation);
		}
	}
	...
	/**
	 * This listener ensures the UI of buttons added to the JButtonBar gets
	 * reset everytime it is changed to a pluggable UI which does not implement
	 * ButtonBarUI.
	 */
		...
		public void propertyChange(PropertyChangeEvent evt) {
			if (evt.getSource() instanceof AbstractButton) {
				...
				if (button.getParent() instanceof JButtonBar
					...
					((ButtonBarUI) ((JButtonBar) button.getParent()).ui)
							.installButtonBarUI(button);
				}
			}
		}
	};
	...
	private static ContainerListener buttonTracker = new ContainerAdapter() {
		...
		public void componentAdded(ContainerEvent e) {
			...
			if (e.getChild() instanceof AbstractButton) {
				((ButtonBarUI) container.ui)
						.installButtonBarUI((AbstractButton) e.getChild());
				((AbstractButton) e.getChild()).addPropertyChangeListener("UI",
		...
		public void componentRemoved(ContainerEvent e) {
			if (e.getChild() instanceof AbstractButton) {
				((AbstractButton) e.getChild()).removePropertyChangeListener(
