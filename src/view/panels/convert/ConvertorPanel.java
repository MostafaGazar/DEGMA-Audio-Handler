...
public class ConvertorPanel {
	...
	public ConvertorPanel(JFrame parent) {
		...
		fromLb = new JLabel(Messages.getString("Constants.from"));
		toLb = new JLabel(Messages.getString("Constants.to"));

		ButtonGroup bg1 = new ButtonGroup();
		mp3_1 = new JRadioButton(Messages.getString("Constants.mp3"));
		wav_1 = new JRadioButton(Messages.getString("Constants.wav"));
		midi_1 = new JRadioButton(Messages.getString("Constants.midi"));
		amr_1 = new JRadioButton(Messages.getString("Constants.amr"));
		bg1.add(mp3_1);
		bg1.add(wav_1);
		bg1.add(midi_1);
		bg1.add(amr_1);
		...
		advancedConvertBt = new JButton(
				Messages.getString("Constants.openAdvancedConverting"));
			...
			public void actionPerformed(ActionEvent e) {
				if (mp3_1.isSelected() && mp3_2.isSelected()
					...
					JOptionPane.showMessageDialog(
							frame,
							Messages.getString("Constants.error_TheInputTypeMustBeDiffrentFromTheOutputType"), Messages.getString("Constants.title_SorryButProcessCanNotCompleted"),
				...
				if (midi_1.isSelected() && mp3_2.isSelected()) {
					JOptionPane.showMessageDialog(
							frame,
							Messages.getString("Constants.error_NotSupportedConversion"), Messages.getString("Constants.title_SorryButProcessCanNotCompleted"),
							JOptionPane.ERROR_MESSAGE);
				}
				// mp3 to wav
	...
	public void setPathes(String type) {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			source = file.getParent() + "\\" + file.getName();
		} else {
			source = null;
		}
		...
		if (source != null && dest != null) {
			...
			} else if ("2".equals(type)) {//wav to mp3
				WaitingDialog holdingWd = new WaitingDialog(frame,
						Messages.getString("Constants.converting"));
				// show the dialog
				holdingWd.setVisible(true);
